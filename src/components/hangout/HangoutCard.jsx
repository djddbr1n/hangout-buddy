...
export function HangoutCard({ hangout, currentUser, friendIds, onOpen, isInvited }) {
  ...
  <motion.button
    ...
    className={`w-full rounded-2xl border text-left overflow-hidden bg-white/90 backdrop-blur-md ${
      isInvited             ? 'border-violet-200/70'
      : hangout.is_surprise ? 'border-pink-200/70'
      : isMine              ? 'border-emerald-200/70'
      : 'border-violet-100/80' // FIXED: Replaced border-white/60 with brand violet tone
    }`}
  >
    {/* Thin top accent bar — sole color signal per card type */}
    <div className={`h-[3px] w-full ${
      isInvited
        ...
        : 'bg-gradient-to-r from-violet-400 to-indigo-300' // FIXED: Swapped amber-400 to brand violet/indigo
    }`} />
    <div className="px-4 py-3.5 flex items-start gap-3">
      ...
      <div className={`w-10 h-10 rounded-xl border flex items-center justify-center text-xl shrink-0 ${
        isInvited
          ...
          : 'bg-violet-50 border-violet-100' // FIXED: Swapped amber-100 to soft brand violet
      }`}>
